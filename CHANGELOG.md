...
- **Notificaciones vía ntfy (#766)**: nuevo canal en la cadena de notificaciones (push + webhook + Telegram + ntfy), para ntfy.sh público o un servidor propio: basta un topic (funciona como contraseña, se recomienda largo y aleatorio) y opcionalmente un token de acceso para topics reservados. Publicación con título y prioridad, cola con reintentos y fail-silent, activable desde la UI sin reiniciar, y botón de prueba que publica de verdad y muestra el error si algo falla. El token nunca sale del servidor.
- **Proxmox VE multi-instancia (#764)**: la integración pasa de un único endpoint a una lista de instancias (clusters o nodos sueltos), cada una con su URL y token de solo lectura; la config anterior migra sola a una instancia "default". El inventario consulta todas las instancias y fusiona VMs/CTs y nodos (con claves por instancia, para que dos clusters con nodos homónimos no se pisen), la topología sella como siempre por MAC, y los nodos de hipervisor llevan su instancia para distinguirlos en la UI. Ajustes muestra la lista con añadir, editar y quitar; los secretos nunca salen del servidor.
...
- **Recarga del certificado TLS sin reiniciar (#769)**: el listener HTTPS con certificados propios (NETPULSE_TLS_CERT/KEY, p. ej. renovados por acme.sh/certbot) cargaba el par una sola vez al arrancar y la renovación solo se servía tras reiniciar el proceso. Ahora el servidor repuebla el par del disco cuando cambia (chequeo de mtime en cada handshake) y la renovación se sirve en el siguiente handshake sin reinicio. Si la renovación está a medio escribir, sigue sirviendo el último par válido y reintenta como mucho cada 30 s. El path on-box autofirmado no cambia: el agente fija el SPKI en el pairing.
- **Alta guiada de dispositivos desconocidos (#772)**: la alerta "dispositivo desconocido" ahora notifica de verdad por los canales configurados (push/ntfy/Telegram, con router, banda y señal) y enlaza a una tarjeta de alta en Clientes: se asigna un nombre (que además silencia futuros avisos para esa MAC), se elige un icono y se puede reservar la IP con el plan de comandos antes de ejecutar; o dejar el dispositivo explícitamente como anónimo, que silencia el aviso sin nombrarlo. Cuando hay clientes conectados sin identificar, la página de Clientes lo anuncia con un banner.
- **Timeline de presencia por cliente (#771)**: el detalle de cada cliente muestra las bandas de presencia de los últimos 7 días (una pista de 24 h por día, coloreada por AP) y el feed de eventos recientes, construidas a partir de los eventos de conexión/desconexión del sondeo y de los AP-STA-CONNECTED de hostapd: un salto a otro AP cierra el tramo y abre otro, así el roaming se ve como un cambio de color. La tarjeta "Roaming (24 h)" de Clientes rankea los equipos que más rebotan entre APs. La retención del historial es configurable (Ajustes → Datos, por defecto 30 días) con una poda horaria que acota ambas tablas de eventos, que crecían sin límite.
- **La vista de la topología recuerda el zoom entre sesiones (#777)**: el nivel de escala (y el desplazamiento) elegido en el modo edición o con los botones/rueda se guarda y se restaura al volver a abrir la topología; "Restablecer vista" vuelve a la vista inicial.
- **La lista de clientes recuerda la vista y el orden (#778)**: el modo lista/tarjetas y la columna y dirección de orden activos se guardan y se restauran al volver a abrir la sección; la búsqueda y los filtros siguen siendo de sesión.

### Fixed

- **Fugas del topic en el canal ntfy (#773)**: auditoría de seguridad del canal (petición para reutilizarlo en NetGrip): el topic ya no aparece en el log de arranque ni en los errores de red (enmascarado), los reintentos solo cubren errores transitorios (5xx/429/red; los 4xx no reintentan), el mensaje se trunca por límite de runa UTF-8 sin cortar caracteres, el botón de prueba exige el canal activado, la publicación en vuelo se cancela al cerrar y el cierre es idempotente.
- **El chip "Servicios" y el fabricante "Desconocido" se quedaban en español con la UI en inglés (#770)**: ambas cadenas venían del backend como etiquetas canónicas en español; ahora se traducen al idioma activo como el resto de etiquetas del health score. El chip además solapaba ligeramente la barra en anchos estrechos.
- **Interfaces AmneziaWG invisibles en el panel WireGuard (#776)**: la detección automática solo consultaba la herramienta estándar `wg`, que no ve las interfaces AmneziaWG (familia netlink propia, herramienta `awg`). Ahora el descubrimiento lista con ambas y el sondeo cae a `awg show` cuando `wg` no puede leer la interfaz; los routers WireGuard puros no se ven afectados (`awg` ausente = "sin interfaces AmneziaWG").
- **El uptime de la consola RTL fallaba con el nuevo formato de `time` (#785)**: firmwares RTLPlayground recientes (KP-9000 v0.1.0-ad9e2ee) responden `Tick counter: 0x…   Sec Counter: 0x…` en vez del contador hex único, y cada sondeo fallaba dejando firmware, uptime y MAC congelados en la UI tras el backoff. El parser acepta ambos formatos (el uptime es el Sec Counter).
...
- **Unidad de temperatura configurable (Celsius o Fahrenheit) (#717)**: las temperaturas se mostraban siempre en Celsius. Ahora Ajustes → "Datos y umbrales" permite elegir °C o °F, una preferencia de solo presentación que se aplica a tarjetas, tabla comparativa, detalle de router, gráfico de rendimiento, topología y temperatura SFP, así como a las descripciones de las alertas de temperatura en vivo (que ahora muestran el umbral efectivo del router). El servidor y el almacenamiento siguen en Celsius, y los umbrales por router se editan en Celsius con etiqueta explícita. Las notificaciones push y el feed de la demo conservan el Celsius.

- **El umbral de alerta por temperatura alta pasa a ser por router (#716)**: antes era un único valor fijo de 65 °C para todos, así que un router que corre más caliente obligaba a subir el umbral global y ocultaba problemas reales en los más frescos. Ahora cada router puede fijar el suyo (columna `temp_threshold` en `routers`, vacío = 65 °C por defecto); la alerta de temperatura y el estado "hot" del dashboard lo usan. En la UI se configura en Ajustes → Dispositivos, y se retira el slider global de "Datos y umbrales" que no gobernaba la alerta real.

- **El panel WireGuard y la topología ya ven los peers de todos los túneles, no solo `wg0` (#713)**: el servidor sondeaba una única interfaz (`WG_INTERFACE`, por defecto `wg0`), así que un gateway con varios túneles solo mostraba los peers de `wg0`. Ahora `WG_INTERFACE` admite `auto` (nuevo default) o vacío para descubrir y agregar los peers de todas las interfaces del router (`wg show interfaces`), y un valor explícito (una interfaz o una lista separada por comas) sigue actuando como filtro, conservando el comportamiento del caso de un solo túnel.
...
- **Los mensajes de rearm y de rotación de token explican ya el motivo real en routers sin agente ni SSH (#718, #719)**: al rearmar, la UI colapsaba cualquier error en un "No se pudo rearmar" genérico; ahora distingue entre sin agente registrado, sin acceso SSH, cooldown, router desconocido, dispositivo no OpenWrt, NetGrip y fallo SSH, con mensajes específicos en el idioma del usuario. Y al regenerar el token de un router sin agente (o gestionado por NetGrip), el aviso ya no dice "reinstala el agente": el servidor indica por qué no se pudo aplicar en caliente y la UI aclara que el token queda guardado y se aplicará cuando el agente se instale o se recupere.

- **El sondeo de WireGuard ya no se cae entero si falla una interfaz o el listado (#714)**: con el sondeo multi-interfaz (#713) un solo `wg show <iface> dump` que salía con status 1 (interfaz caída o inaccesible) abortaba toda la agregación y dejaba el panel WireGuard en "inactivo" permanente, logueando "no disponible" en cada tick. Ahora el fallo de una interfaz se salta y se agregan los peers de las que responden; si `wg show interfaces` no está soportado (o falla), se cae al default histórico `wg0`; y si ninguna interfaz responde, el panel muestra el túnel inactivo sin loguear error en cada tick.
- **El feed de alertas ya no muestra español para los tipos que faltaban por migrar (#712)**: los emisores que seguían generando literales sin `type`/`vars` (recuperaciones de agente, router y puerto, acceso SSH perdido/recuperado, handshake de WireGuard, los eventos del beacon del switch, el diagnóstico SFP y el auto-rearme) llevan ahora su slug estable y sus variables, y el frontend traduce el título y la descripción en el idioma del usuario. Los literales se mantienen como respaldo y el dedup/silence (que usan el título) no cambian.
...
- **Las notificaciones push de alertas se traducen al idioma del dispositivo (#689)**: el payload viajaba con los literales en español, así que un usuario con la app en inglés recibía los avisos en español. Ahora el server añade el slug del tipo y sus variables (`type`/`vars`) y el service worker traduce con el catálogo de la app (`alerts.types.*` / `alerts.hints.*`) según el idioma del navegador, manteniendo los literales como fallback.
...
- **Sección de ayuda dentro de la app (#484)**: nueva página de guía rápida con recorridos guiados para los cinco flujos que un admin necesita el primer día: primer arranque y contraseña, alta de un router, instalación del agente (el botón de un clic primero y el one-liner con token de pairing como alternativa manual), lectura del plan de canales y upgrade de firmware. Los pasos citan las etiquetas reales de la UI renderizadas como chips y escritas en los mismos ficheros de i18n que la interfaz, de modo que la ayuda deriva junto con ella; sin capturas bitmap, que envejecen mal. Paridad completa ES/EN, ítem de navegación y entrada en la paleta de comandos.

### Fixed

- **CSP permite estilos inline para Radix y framer-motion (#485)**: cada apertura de un diálogo (paleta de comandos incluida) logueaba una violación de Content Security Policy y las transiciones de entrada/salida no se ejecutaban. Radix (posicionado) y framer-motion (transform/opacity) escriben atributos style con valores dinámicos imposibles de hashear, así que `style-src` gana `'unsafe-inline'`; `script-src` sigue estricto (`'self'`) y la app no renderiza HTML controlado por el usuario.
